(() => {
  ...
  function localState(tagToAdd = null) {
    ...
    if (tagToAdd) {
      const phrases = {
        stone: [38, 43, 45, 50],
        flower: [62, 65, 69, 74, 69, 65],
        cloud: [69, 74, 76, 81],
        spiral: [57, 62, 60, 55, 57],
        sun: [74, 78, 81, 86],
        moon: [53, 57, 60, 64, 60],
        lightning: [79, 86, 91, 83],
      };
      const priorTouches = stored.filter((event) => event.tag === tagToAdd).length;
      const phrase = phrases[tagToAdd] || [62];
      const velocityWave = [76, 94, 84, 108, 88, 98];
      const durationWave = [0.42, 0.7, 0.32, 0.92, 0.5, 0.76];
      addedEvent = {
        id: `${Date.now()}-${Math.random()}`,
        tag: tagToAdd,
        note: tagToAdd === "rest" ? null : phrase[priorTouches % phrase.length],
        velocity: tagToAdd === "lightning" ? 116 : velocityWave[stored.length % velocityWave.length],
        duration: tagToAdd === "lightning" ? 0.12 : tagToAdd === "cloud" ? 1.6 : durationWave[stored.length % durationWave.length],
        createdAt: new Date().toISOString(),
      };
      stored.push(addedEvent);
